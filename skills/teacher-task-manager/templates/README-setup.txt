...
학생 수신 계정은 학교 계정(@goedu.kr)을 권장하며 일반 Gmail(@gmail.com)도 사용할 수 있습니다.
선생님 로그인과 발신은 @goedu.kr 계정만 지원합니다.
학교 계정은 다음 세 단계를 모두 마쳐야 생깁니다.
1. 교육디지털원패스 가입: 담임 선생님은 회원 정보 → 학생 회원 가입 → 학생 회원 가입 현황에서
   원패스 ID를 확인할 수 있습니다. 이것만으로 Google 계정이 생긴 것은 아닙니다.
2. 경기도교육청 교육용 클라우드 지원시스템 가입: 학생이 원패스 ID로 추가 가입합니다.
3. 교육용 클라우드 안에서 Google Workspace 사용을 별도로 신청합니다.
   완료 후 학교 이메일은 보통 원패스 ID@goedu.kr입니다.
학생 준비 도움말: https://www.goedu.kr/bbs/2/view/55
선생님이 https://chat.google.com/ 의 새 채팅 → 한 명 이상을 추가하세요 칸에 학생 이메일을
붙여 넣어 직접 초대해 주세요. 한 줄에 하나씩 또는 쉼표와 띄어쓰기로 구분합니다.
띄어쓰기만으로 구분하면 인식되지 않습니다. 메시지를 한 번 보낸 뒤 위쪽 대화 이름 →
이 채팅을 스페이스로 전환에서 짧은 학급 이름을 정해 주세요. Teacher Manager로 돌아와
다시 불러오기 후 학급 단체톡방을 선택합니다. 학생 가입·초대·삭제는 자동으로 하지 않습니다.
학생명단의 학생 Google 이메일 칸에는 학생 한 명당 주소 한 개만 입력합니다.
...
3. 로그인 전에 현재 상태를 확인합니다:
   ...
   처음 한 번 설정하기 -> 처음 설정 한 번에 끝내기를 누릅니다.
   이 한 번의 설정 안에 AI 출결 입력 연결 작업도 포함됩니다.
