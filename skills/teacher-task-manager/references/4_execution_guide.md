...
Calendar·Tasks·Sheet·Docs·출결·Google Chat처럼 Google 자료를 읽거나 바꾸기 전에 설치된 안전한 GWS 명령 파일로 현재 계정을 다시 확인한다. 로그인이 유효하지 않거나 이메일 주소 한 개를 확인할 수 없으면 Google 자료 작업을 시작하지 않는다. 주소의 도메인은 제한하지 않으며, 기존 자료와 로그인 계정이 일치하는지, 해당 자료에 접근할 권한이 있는지는 따로 확인한다.

선생님 로그인·발신과 학생 Google Chat 수신은 도메인에 관계없이 Google 계정 주소를 허용한다. 실제 Google Chat 사용 가능 여부는 Google 권한과 조직 정책 및 연결·발송 결과로 확인한다. 학생명단의 이메일 칸에는 학생 한 명당 주소 한 개만 적는다.
