...
교사 로그인에는 도메인에 관계없이 Google에 로그인할 수 있는 계정을 허용한다. 이미 Google 계정이 있으면 아래 가입을 건너뛴다. 경기도교육청 학교 계정을 새로 준비할 때만 다음 세 가지를 마친다.
...
Google Chat 학생 수신에는 학교별 계정과 개인 Gmail 등 모든 Google 계정 주소를 허용한다. 경기도교육청 학교 계정을 새로 준비할 때는 **교육디지털원패스 가입 → 경기도교육청 교육용 클라우드 지원시스템 추가 가입 → 클라우드 안에서 Google Workspace 별도 가입**을 모두 마친다. 원패스의 **회원 정보 → 학생 회원 가입 → 학생 회원 가입 현황**에서 확인하는 것은 원패스 ID다. 원패스 ID로 클라우드에 가입하고 Google Workspace 신청까지 완료해야 학교 Google 계정이 생긴다. 학교 이메일은 보통 `원패스 ID@goedu.kr`이다. 원패스 ID 확인만으로 Google 계정 가입이 끝났다고 안내하지 않는다.

선생님이 [Google Chat](https://chat.google.com/)의 **새 채팅 → 한 명 이상을 추가하세요** 칸에 학생 이메일을 한꺼번에 붙여 넣어 직접 초대한다. 이메일은 한 줄에 하나씩 또는 쉼표와 띄어쓰기로 구분하며, 띄어쓰기만으로 구분하지 않는다. 메시지를 한 번 보낸 뒤 **위쪽 대화 이름 → 이 채팅을 스페이스로 전환**에서 짧은 학급 이름을 정한다. Teacher Manager로 돌아와 **다시 불러오기** 후 학급 단체톡방을 선택한다. Teacher Manager는 학생 가입·초대·삭제를 자동으로 하지 않는다.

Teacher Manager는 경기도교육청 소속 교사 전용이다. Google 로그인으로 계정을 확인하며 주소의 도메인을 제한하지 않는다. 이메일만으로 교사와 학생의 신분까지 구별할 수는 없다. 학생에게 프로그램을 쓰게 하거나 선생님의 출결 Sheet 편집 권한을 주지 않는다. 학생은 Google Chat 학급 단체톡방에만 초대한다. 출결 Sheet의 `설정` 탭에는 Chat 연결값이 있으므로 학생이나 믿을 수 없는 공동 편집자와 공유하지 않는다.
...
- 홈은 `내 정보`, `시간표`, `연결`, `설정` 네 카드로 구성되고, 처음 설정은 시작 전 준비 → Google 로그인 → 내 정보 → 하루 일과 → 시간표 → 이 컴퓨터 설정 → Google 연결 → 학생 계정 준비 → 모두 저장 순서로 진행한다. 문제가 있으면 홈 카드에 `확인 필요`와 `N/M 정상` 숫자가 붙고, 실제 원인은 해당 화면의 입력칸 아래에 정확한 문장으로 보인다.
...
- 출결 자료를 처음 만드는 곳은 처음 설정 7단계의 출결 탭 `저장하기` 한 곳이다. 기존 연결 기록이 있으면 새 Sheet를 만들지 않고 그대로 재사용하며, 저장을 반복해도 자료가 중복 생성되지 않는다.
- 마지막 `모두 저장`은 내 정보·시간표·메신저 설정 저장 → 설정 읽기 → 도우미 시작만 한다. 출석부를 만들거나 다른 출석부로 연결하지 않는다.
...
컴퓨터 로그인에는 감지기 관리 권한과 시트 안 외부 요청 권한을 넣지 않는다. 새 정본이나
연결 표시가 손상된 정본에서만 선생님이 그 Sheet의 `[처음 한 번 설정하기] -> [처음 설정 한 번에 끝내기]`를
누른다. 이때 Sheet가 자기 권한인 `https://www.googleapis.com/auth/script.scriptapp`과
`https://www.googleapis.com/auth/script.external_request`를 필요할 때 직접 안내한다. 확인 표시가
정상이면 나중 업데이트에서는 기존 감지기 하나를 그대로 유지하고 이 동작을 다시 요구하지 않는다.
권한 허용을 취소하면 현재 Sheet와 현재 자동화는 그대로 보존한다. 출석부 연결을 바꾸거나 예전
파일을 휴지통으로 옮기지 않는다.
...
**[중요] 로그인 계정은 Google에서 인증된 계정이어야 한다.** 학교별 도메인과 개인 Gmail 등 주소의 도메인에 관계없이 설정과 Google 작업을 시작할 수 있다. 주소 허용만으로 Google Chat 자동 발송 성공을 보장하지 않으며, 실제 연결·발송 결과로 확인한다. 기존 출석부·캘린더·Tasks의 계정 일치와 접근 권한 확인은 그대로 유지한다.

`& $gws auth status` 결과의 `user` 값을 반드시 확인한다. 로그인이 유효하고 이메일 주소 한 개를 확인할 수 있는지 확인한다. 확인할 수 없으면 설정 질문으로 넘어가지 말고 아래처럼 안내한 뒤 다시 로그인한다.

> 지금 로그인된 계정은 `{user}`입니다. 사용할 Google 계정으로 다시 로그인해 주세요.

로그인 요청이 있으면 같은 scope로 `& $gws auth login`을 다시 실행하고, 브라우저 계정 선택 화면에서 본인의 Google 계정을 고르도록 안내한다. 로그인이 끝나면 `& $gws auth status`로 `user` 값이 본인의 Google 계정인지 다시 확인한 뒤에만 다음 단계로 넘어간다. 로그인 계정을 확인할 수 없다면 Calendar·Tasks·Sheet·Docs·출결·Chat 작업과 처음 설정 완료를 시작하지 않는다.
...
6. `학생명단` 시트를 채우게 한다. 열은 왼쪽부터 `번호`(A), `이름`(B), `학생 Google 이메일`(C) 3개다.
   - **선생님이 직접 채우는 열**
     - `번호`(A), `이름`(B): 나이스나 엑셀 명단에서 그대로 붙여넣으면 된다.
     - `학생 Google 이메일`(C): 학생 한 명당 Google 계정 이메일 주소 한 개를 적는다. 주소의 도메인은 제한하지 않는다. 주소가 없거나 주소 한 개의 형식이 아니면 보내지 않고 발송기록에 이유를 남긴다. Google Chat 초대 칸과 달리 이 칸에는 여러 이메일을 한꺼번에 넣지 않는다. 별도의 사용 여부(Y/N) 표시는 없다.
   - `번호+이름`은 학생명단에 따로 적지 않는다. 월별 시트에서 학생을 고를 때 A열 번호와 B열 이름을 자동으로 합친 `3김민수` 모양으로 보인다.
...
- 자동 분석으로 들어온 안내는 `대기` 상태로 대장에 저장한다. 대기라고 자동 발송되는 것은 아니며 선생님이 발송 버튼을 눌러야 전송된다.
- 보내지 않을 줄은 `제외`로 바꾼다. `제외`, `보냄`, `실패` 상태는 종례 발송 대상이 아니다.
...
일반 업데이트·재설치·설정 저장에서는 새 출석부를 만들지 않는다. 먼저 컴퓨터에
저장된 현재 Google Sheet 전체 번호를 읽고, 그 번호의 파일이 현재 학교 계정 소유,
정식 파일 이름, 학년도, 스프레드시트 종류, Teacher Manager 정식 표식을 모두
충족하는지 확인한다.

컴퓨터의 현재 연결 기록이 없거나 옛 형식이라 정식 확인 표시를 통과하지 못하면 새 파일을 만들지 않고 Teacher Manager의
`연결 확인/바꾸기`를 연다. 화면에는 정식 표식이 있는 후보만 보여 주며, 후보마다
정식 이름, 최근 사용 시각, 출결 기록 줄 수, `TM-XXXXXX-XXXXXX` 연결 확인번호를
보여 준다.

1. 사용할 Sheet에서 `처음 한 번 설정하기 -> 연결 상태 확인`을 눌러 연결
   확인번호를 눈으로 확인한다.
2. Teacher Manager 후보의 번호와 같으면 `이 출석부 사용`을 누른다.
3. 자동 목록이 헷갈리면 Sheet의 연결 확인번호를 복사해
   `확인번호로 직접 연결`에 붙여 넣는다.
4. Teacher Manager는 번호가 정확히 하나의 정식 후보와 맞을 때만, 그 후보의 전체
   Google Sheet 번호를 다시 확인한 뒤 컴퓨터의 현재 연결 기록을 마지막에 바꾼다.

연결 확인번호는 사람이 비교하기 위한 짧은 표시다. 실제 실행에서는 저장된 Google
Sheet 전체 번호만 사용한다. 파일 이름만 보고 자동 연결하거나, 선택하지 않은 Sheet를
합치거나, 이름을 바꾸거나, 휴지통으로 옮기지 않는다. Google Sheet 삭제는 사용자가
직접 한다.
...
2. 새 정본이나 연결 확인 표시가 없는 정본이면 상단
   `[처음 한 번 설정하기] -> [처음 설정 한 번에 끝내기]`를 누른다. 처음 한 번은 Google
   권한 승인 화면이 뜰 수 있다. 이 설정 안에 AI 출결 입력 연결 작업도 포함된다.
3. Sheet가 같은 정본을 위한 편집 감지기를 정확히 하나로 맞추고, 다시 읽어 하나임을
   확인한 뒤 비밀값 없는 연결 확인 표시를 마지막에 남긴다.
4. Gemini API key는 컴퓨터의 Teacher Manager 연결 화면에서 저장한 값을 `설정` 탭에서
   읽는다. 시트에서 다시 묻지 않는다.
5. 월 시트 1행의 빈 입력칸에 `7월 25일 홍길동 감기로 병결`처럼 적고 Enter를 누른다.
   입력 예시는 칸에 미리 적혀 있지 않고, B1~M1로 합쳐진 입력칸의 메모에서 확인한다.

이미 적힌 월별 출결도 같은 입력칸에서 고칠 수 있다. `19일 백시은 3교시 아니고
4교시야`, `19일 백시은 사유가 감기가 아니라 복통이야`처럼 날짜·학생과 바꿀
내용을 함께 적는다. 날짜, 학생, 구분, 종류, 사유, 교시, 신고서, 첨부를 고칠 수
있으며, 같은 학생의 여러 내용을 한 문장으로 함께 고쳐도 된다. `25일 애들 사유
전부 복통으로`, `25일 애들은 전부 다 냈어`처럼 날짜의 기존 학생 줄 전체도 바꿀
수 있다. 제출 대상을 따로 말하지 않은 `냈어`·`미제출`은 첨부 상태로 본다.

기존 줄 수정은 날짜와 학생, 필요하면 문장에 적힌 현재 값까지 맞는 줄만 바꾼다.
대상이 없거나 여러 줄로 남으면 새 줄을 만들지 않는다. 바꾸라고 한 출결 내용만
고치고 Google Chat 기록과 AI 표시는 보존한다. 모든 변경을 다시 읽어 확인한 뒤
입력칸만 조용히 비우며, 성공 안내 창이나 별도의 `적용` 입력은 요구하지 않는다.
날짜나 학생을 고치면 날짜순과 같은 날짜 안 학생 번호순으로 다시 정리한다.

같은 메뉴를 다시 눌러도 두 번째 감지기를 만들지 않는다. 확인 표시가 정상이면 이후 일반
업데이트는 기존 감지기 하나를 그대로 유지하며 메뉴 동작을 반복해서 요구하지 않는다.
감지기를 만들고 정리하는 데 Sheet 쪽
...
업데이트가 중간에 멈췄으면 같은 `출결 기능 업데이트` 버튼으로 현재 출석부의 기능만
이어서 확인하며 새 Google Sheet를 만들지 않는다. 직접 고친 스크립트는 자동으로 덮지 않는다.
...
1. `출결 업무 자동화`: 월별 시트에서 미제출 서류 행을 선택하고 `선택 행 미제출 서류 Google Chat 개인톡 보내기`를 누른다. 결과는 월별 시트 맨 끝의 두 줄 제목인 Google Chat 발송상태, Google Chat 발송시각, Google Chat 결과에서 확인한다.
...
LLM 입력(공문 분석 등)으로 담임 Tasks를 만들었으면 같은 안내 문장을 쪽지 대장에도 저장한다. 학급 전체 안내는 `메신저 단체톡 내용`에, 특정 학생 안내는 `메신저 개인톡 내용`에 `자동분석` / `대기` 상태로 넣는다. 대기 상태만으로 자동 발송하지는 않으며 학생에게 바로 발송되지 않는다. 시트 ID는 `attendance-install.generated.json`의 `spreadsheet_id`를 쓰고, 먼저 같은 날짜 줄을 읽어 같은 내용이 이미 있으면 추가하지 않는다. 선생님을 Google Sheet 메뉴로 다시 보내지 않는다.
...
& $gws sheets spreadsheets values append --params '{"spreadsheetId":"<출결 시트 ID>","range":"메신저 단체톡 내용!A1","valueInputOption":"RAW","insertDataOption":"INSERT_ROWS"}' --json '{"majorDimension":"ROWS","values":[["<오늘 YYYY-MM-DD>","기타","<안내 문장>","자동분석","대기","",""]]}'
```
...
1. 등록 전에 `학생명단` 시트에서 번호(A)와 이름(B)만 읽는다. C열(학생 Google 이메일)은 대화 컨텍스트로 가져오지 않는다.
...
5. 쪽지 종류는 `출결서류`, `준비물`, `개별안내`, `상담/확인`, `기타` 중에서 고르고 애매하면 `개별안내`. 상태는 기본 `대기`이며 대기만으로 자동 발송되지는 않는다. 연결 표시는 빈 값으로 둔다.
...
Google Chat 발신 연결과 학생 수신에는 도메인에 관계없이 Google 계정을 허용한다. 실제 Chat 사용 가능 여부는 Google 권한과 조직 정책 및 연결·발송 결과로 확인한다. 첫 개인 메시지는 수신자의 채팅 요청 수락이 필요할 수 있다. 경기도교육청 학교 계정을 새로 준비하는 학생은 위의 원패스·교육용 클라우드·Google Workspace 가입 세 단계를 모두 마친다. 선생님은 Google Chat에서 학생 계정을 직접 초대하여 학급 단체톡방을 준비한다.

학생 초대와 방 참여는 선생님이 수작업으로 진행한다. 스킬은 학생 가입·초대·삭제를 자동으로 하지 않는다.

개인 DM은 `학생명단`의 학생 Google 이메일을 기준으로 보내되 Google 계정 이메일 주소 한 개만 대상으로 삼으며 도메인은 제한하지 않는다. 주소가 없거나 주소 한 개의 형식이 아니면 보내지 않고 발송기록에 이유를 남긴다. 수신 계정과 선생님 로그인·발신 계정은 각각 확인한다.

설치 뒤 선생님에게 이렇게 안내한다.

> Google Sheet의 `메신저 개인톡 내용`과 `메신저 단체톡 내용`을 확인하세요.
> 자동으로 들어온 줄은 `대기` 상태입니다. 보내지 않을 줄만 `제외`로 바꾸면 됩니다. 대기 줄도 발송 버튼을 누르기 전에는 학생에게 전송되지 않습니다.
...
- 처리한 메시지 목록에서 일부 항목만 실패했고 다시 쓸 정보가 안전하게 남아 있으면
  실패 기록 안에 `실패한 항목 다시 시도`가 보인다. 문제를 해결한 뒤 이 버튼을 누르면
  이미 성공한 Calendar·Tasks는 건너뛰고 실패한 항목만 다시 처리한다. Brity로 돌아가
  단축키를 다시 누르게 안내하지 않는다.
- 재시도 전의 기록과 재시도 결과는 복구용 원본에 모두 남긴다. 홈의 `처리한 메시지`에는
  두 기록을 따로 늘어놓지 않고, 처음 성공한 항목과 재시도 결과를 합친 최신 결과 한 건만
  보여 준다. 재시도가 성공하면 성공 안내와 전체 처리 내용이 계속 남고, 이전의 빨간 실패
  문구와 Brity에서 다시 단축키를 누르라는 안내는 보이지 않는다. 이미 만들어졌는지
  확실하지 않거나 다시 쓸 정보가 부족한 기록에는 재시도 버튼을 보이지 않는다.
- 메시지에 학생·학급 안내가 있으면 출결 자동화 시트의 `메신저 개인톡 내용`/`메신저 단체톡 내용` 시트에 상태 `대기`로 들어간다. 대기 상태로 옮긴 것은 아직 학생에게 보낸 것이 아니다. 선생님이 발송 버튼을 눌러 마지막 확인을 마친 뒤에만 보낸다. 출결 자동화 시트가 없으면 `옮기지 못함 · 처음 설정 필요`로 남긴다.
...
- 원문 세부사항을 버리지 않는다. 장소, 대상, 주요 안건, 유의사항, 문의, 발신, 첨부파일이 있으면 설명칸에 각각 살려 쓴다. Brity 메신저에서 확인한 첨부파일은 업무·학사일정을 가리지 않고 만들어지는 모든 캘린더 설명에 한 줄씩 빠짐없이 넣는다. 파일명 맨 앞이 `(기관명-문서번호 (첨부) 부서명)` 또는 `(기관명-문서번호 (본문) 부서명)` 모양이면 그 행정 접두어만 빼고, 뒤의 문서 제목·붙임 표시·확장자는 그대로 쓴다.
