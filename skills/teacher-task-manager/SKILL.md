...
같으면 그대로 연결한다. 이 프로그램이 배포했던 믿을 수 있는 옛 판이면 출결 연결은
보존한 채 출결 탭에 한 줄 안내와 `문제 해결` 버튼만 보여 준다. `문제 해결`을 누르면
먼저 읽기만 해서 상태를 다시 확인하고, 정식 예전 판으로 확인된 뒤 작은 창의
`업데이트`를 선생님이 따로 승인했을 때만 같은 Apps Script 프로젝트 안에서 바꾼다.
마지막 연결에서 멈췄던 경우에는 같은 자리에 `마무리` 버튼을 보여 주며, 이미 만든
판을 다시 연결할 뿐 새 Sheet나 새 판을 만들지 않는다. 출처를 확인할 수 없거나 선생님이 직접
...
이 절차는 시트 스크립트 `5.11.0`부터 이 모양이다. 예전 공개판을 그대로 쓰는 시트는 Teacher Manager 출결 탭에 한 줄 안내와 `문제 해결` 버튼을 보여 주고, 누르면 먼저 읽기만 해서 상태를 확인한다. 정식 예전 판으로 확인된 뒤 작은 창의 `업데이트`를 선생님이 따로 눌렀을 때만 같은 Apps Script를 갱신한다. 마지막 연결에서 멈췄던 시트는 `마무리` 버튼으로 이미 만든 판을 다시 연결할 뿐 새 Sheet나 새 판을 만들지 않는다. 직접 고친 스크립트는 자동으로 덮지 않는다. 감지기를 만들고 정리하는 데 `https://www.googleapis.com/auth/script.scriptapp` 권한이 필요해서, 이미 쓰던 선생님도 처음 한 번 다시 승인해야 한다. `5.9.0` 이전 판에서 시트 메뉴로 직접 키를 넣어 둔 선생님은 그 값이 그대로 쓰인다.
