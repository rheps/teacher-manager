...
연결 표시가 손상된 정본에서만 선생님이 그 Sheet의 `출결 업무 자동화 -> AI 출결 입력 연결 확인`을
한 번 누른다. 이때 Sheet가 자기 권한인 `https://www.googleapis.com/auth/script.scriptapp`과
...
재실행, 일반 설정 저장은 이 번호를 바꾸거나 새 Google Sheet를 만들면 안 된다. 일반 업데이트·저장·새 학년도 시작에서는 기존 Google Sheet를
휴지통으로 옮기거나 이름을 바꾸지 않는다. 시트
...
보인다. 사용자가 확인한 `출결 시트 하나로 정리`에서만 다음 순서로 한 번 정리한다.

1. 현재 연결 파일과 정확히 같은 이름이며 같은 학교 계정이 소유한 출석부를 먼저 읽기만 해서 확인한다.
2. 월별 출결, 메시지 대장, 발송기록은 중복처럼 보여도 빼지 않고 새 정식 이름의 후보에 순서대로 이어 붙인다.
3. 학생명단·휴일·설정은 현재 연결 파일의 값만 기준으로 옮긴다.
4. 새 Apps Script의 부모 파일, 올린 코드, 배포판, 설정값과 AI 입력 감지기를 확인한다.
5. 후보 파일을 끝까지 확인한 뒤 현재 연결번호를 마지막에 바꾼다.
6. 새 정본 연결을 다시 확인한 뒤 정확히 같은 이름의 옛 Google Sheet를 복구 가능한 Google Drive 휴지통으로 옮긴다. 영구 삭제하지 않는다.
7. Docs 서식·증빙 폴더·Tasks 목록은 그대로 이어 쓴다.

원본이 없거나 확인 결과가 모호하면 자동으로 고르지 않고 멈춘다. 직접 고친 Apps Script는
자동으로 덮어쓰지 않는다. 코드 갱신이 필요해도 상태를 읽어 정식 코드인지 확인하고,
선생님이 따로 승인했을 때만 같은 Apps Script를 바꾼다.

이 흐름의 자동 확인은 가짜 Google 응답으로 안전 경계만 검사한다. 학교 계정의 실제 Google
Sheet·자동화·휴지통 이동은 학교에서 직접 확인하기 전까지 **실제 동작 미검증**이다.
...
2. 새 정본이나 연결 확인 표시가 없는 정본이면 상단
   `출결 업무 자동화 -> AI 출결 입력 연결 확인`을 한 번 누른다. 처음 한 번은 Google
   권한 승인 화면이 뜰 수 있다. `처음 설정 한 번에 끝내기`도 같은 확인 동작을 포함한다.
