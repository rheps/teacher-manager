...
Google Chat 학생 수신 계정은 학교 계정(`@goedu.kr`)을 권장하며 일반 Gmail(`@gmail.com`)도 허용한다. 학교 계정을 준비할 때는 **교육디지털원패스 가입 → 경기도교육청 교육용 클라우드 지원시스템 추가 가입 → 클라우드 안에서 Google Workspace 별도 가입**을 모두 마친다. 원패스의 **회원 정보 → 학생 회원 가입 → 학생 회원 가입 현황**에서 확인하는 것은 원패스 ID다. 원패스 ID로 클라우드에 가입하고 Google Workspace 신청까지 완료해야 학교 Google 계정이 생긴다. 학교 이메일은 보통 `원패스 ID@goedu.kr`이다. 원패스 ID 확인만으로 Google 계정 가입이 끝났다고 안내하지 않는다.
...
6. `학생명단` 시트를 채우게 한다. 열은 왼쪽부터 `번호`(A), `이름`(B), `학생 Google 이메일`(C) 3개다.
   - **선생님이 직접 채우는 열**
     ...
     - `학생 Google 이메일`(C): 학생 한 명당 정확한 `@goedu.kr` 또는 `@gmail.com` 주소 한 개를 적는다. 학교 계정을 권장한다. 주소가 없거나 지원하지 않는 주소이면 보내지 않고 발송기록에 이유를 남긴다. Google Chat 초대 칸과 달리 이 칸에는 여러 이메일을 한꺼번에 넣지 않는다. 별도의 사용 여부(Y/N) 표시는 없다.
...
Google Chat 발신은 선생님의 `@goedu.kr` 계정으로 한다. 학생 수신은 `@goedu.kr` 또는 `@gmail.com`을 허용하며 학교 계정을 권장한다. 학교 계정을 준비하는 학생은 위의 원패스·교육용 클라우드·Google Workspace 가입 세 단계를 모두 마친다. 선생님은 Google Chat에서 학생 계정을 직접 초대하여 학급 단체톡방을 준비한다.

학생 초대와 방 참여는 선생님이 수작업으로 진행한다. 스킬은 학생 가입·초대·삭제를 자동으로 하지 않는다.

개인 DM은 `학생명단`의 학생 Google 이메일을 기준으로 보내되 정확한 `@goedu.kr` 또는 `@gmail.com` 주소 한 개만 대상으로 삼는다. 주소가 없거나 지원하지 않는 주소이면 보내지 않고 발송기록에 이유를 남긴다. 수신 계정 허용은 선생님 로그인·발신 제한과 별개이며, Gmail로 선생님 Google 작업을 시작하지 않는다.
